...
/**
  * In this part you can practice your FP skills with
 *
 *
 * small exercises.
...
object FPPractice {
    ...
    /** Q21 (5p)
      ...
      */
    def passingStudents(grades: List[List[Int]]): Int = grades.filter(x=>x.sum/x.size>=5.75 && x.filter(x=>x<4).isEmpty).size
    ...
    def checkProp(xs: List[Int]): Boolean = xs match{
        case i :: tail => if(i == sum(tail)) true
                else
            false
    }

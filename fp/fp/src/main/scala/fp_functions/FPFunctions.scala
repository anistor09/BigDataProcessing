...
object FPFunctions {
  ...
  def recFlat(xs: List[Any]): List[Any] =
     ...
     case i :: tail => i match {
       ...
       case j :: secondTail => recFlat(List(j):::secondTail)::: recFlat(tail)
       case _ => i :: recFlat(tail)
     }
    ...
    /** Q18 (5p)
      ...
      * @return the result of folding `xs` with `f`.
      */
    def foldR[A, B](xs: List[A], f: (A, B) => B, init: B): B =  foldL(reverseList(xs),(x : B, y: A) =>f(y,x),init)

    def reverseList[A](xs: List[A] ): List[A] = xs match{

        case i :: tail =>   reverseList(tail) ::: List(i)
    ...
    def zip[A, B](xs: List[A], ys: List[B]): List[(A, B)] = (xs, ys) match {
      ...
      case (i:: tailXS, j:: tailYS) => (i,j)::zip(tailXS,tailYS)
